    ...
    def run(self):
        ...
        if out_putFile.exec_() == QDialog.Accepted:
            ...
            if result == 1:
                ...
                if dbg:
                    dbgMsg(f"Temporary output folder: {out_folder}")
                ...
                input_dataset.SetGeoTransform(
                    [ULx, pixel_width, 0, ULy, 0, pixel_height]
                )
                ...
                if dbg:
                    dbgMsg("*--- GDAL info ---*")
                    dbgMsg(
                        "This is the *first* instance of the dataset, directly after writing image to file from QGIS"
                    )
                    dbgMsg(
                        "Driver: {}/{}".format(
                            input_dataset.GetDriver().ShortName,
                            input_dataset.GetDriver().LongName,
                        )
                    )
                    dbgMsg(
                        "Size: {} x {} x {}".format(
                ...
                if SourceCRS != "EPSG:4326":
                    ...
                    driver = gdal.GetDriverByName("GTiff")
                    driver.CreateCopy(output_geofile, reproj_file, 0)
                ...
                if dbg:
                    dbgMsg("*--- GDAL info ---*")
                    dbgMsg(
                        "This is the *Second* instance of the dataset, after writing image to file from QGIS and potentially reprojecting."
                    )
                    dbgMsg(
                        "Driver: {}/{}".format(
                            indataset.GetDriver().ShortName,
                            indataset.GetDriver().LongName,
                        )
                    )
                    dbgMsg(
                        "Size: {} x {} x {}".format(
                            x_extent, y_extent, indataset.RasterCount
                        )
                    )
                ...
                if optimize:
                    ...
                    if (tile_width, tile_height) == (1, 1):
                        ...
                        if dbg:
                            dbgMsg(
                                "Done, optimal tile size: {} x {}".format(
                ...
                with open(os.path.join(out_folder, "doc.kml"), "w") as kml:
                    ...
                    kml.write(
                        "    <name> {} </name>\n".format(
                            in_file.encode("UTF-8").decode("utf-8")
                        )
                    )
                    ...
                    for r in range(1, n_rows + 1):
                        ...
                        for c in range(1, n_cols + 1):
                            ...
                            if dbg:
                                dbgMsg(
                                    f"Calculated tile extent: N:{N}, S:{S}, E:{E}, W:{W}"
                                )
